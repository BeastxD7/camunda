...
async function getOptimizeDashboardIdsController(req, res, next) {
    try {
...
async function getOptimizeReportIdsController(req, res, next) {
    try {
        const collectionId = parseCollectionId(req.query.collectionId);
        if (collectionId === undefined) {
            return apiError(res, 400, "Invalid request", {
...
async function exportOptimizeDashboardDefinitionsController(req, res, next) {
    try {
        ...
        return apiSuccess(res, 200, "Optimize dashboard definitions exported successfully", data);
    }
    catch (error) {
        next(error);
    }
